...
| `secret` | Presence marks the entry as Secret-backed. Has `key` (single Secret key) OR `key_per_service` (different Secret key per service). Optional `generate` is a shell command stdout-substituted into Secret bootstrap. Without `secret`, the env block uses literal `value:` |
...
- One field per `service_config.E` entry where `S in E.used_by`:
  - `E.required` and no default → field with no default (Pydantic enforces).
  - `E.default` set → field with that default.
  - `E.default_per_service` set → field with `E.default_per_service[S]`.
  - `E.secret` set → field type is `SecretStr` (or whatever `python_type` says — `secret` and `python_type` are independent; `secret = { ... }` only affects how the Secret/env block is rendered, not the Python type).

The generated file carries a `# AUTOGENERATED — do not edit. Regenerate with `loom config codegen`.` header. CI runs `loom config codegen --check` and fails if the working tree differs.
...
{%- else %}
    value: {{ entry.value_for(service) | tojson }}
{%- endif %}
{%- endfor %}
{%- endmacro %}
```
...
Hand-written env entries (`LOOM_ENV=production`, `LOOM_CP_ADMIN_SECRET_FILE`, `LOOM_CP_LLM_GATEWAY_URL`) either move into the schema (preferred for `LLM_GATEWAY_URL`) or stay appended after the macro call as template-local overrides (acceptable for `LOOM_ENV` and file-mount paths that aren't operator-tunable).
