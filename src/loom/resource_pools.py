...
def _aggregate(pools: list[dict[str, object]], *, queued_tasks: int) -> dict[str, int]:
    active_workers = sum(int(pool["active_workers"]) for pool in pools)
    total_slots = sum(int(pool["total_slots"]) for pool in pools)
    occupied_slots = sum(int(pool["occupied_slots"]) for pool in pools)
    return {
        "active_workers": active_workers,
        "total_slots": total_slots,
        "occupied_slots": occupied_slots,
        "free_slots": max(0, total_slots - occupied_slots),
        "running_tasks": sum(int(pool["running_tasks"]) for pool in pools),
        "starting_tasks": sum(int(pool["starting_tasks"]) for pool in pools),
    ...
    *,
    ...
    for state, worker_id, requires_caps in trial_rows:
        ...
        if state not in {"claimed", "running"} or worker_id is None:
            continue
        key = worker_to_key.get(worker_id)
        if key is None:
            continue
