...
the agent catalog, but agents still remain `setup needed` in the UI until
the operator flips catalog readiness after a live platform trial smoke.
If the audit reports `blocked`, the image is still missing a declared
runtime dependency or the adapter contract no longer matches the upstream
package. The `openhands-sdk` adapter intentionally uses Loom's
